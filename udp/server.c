...
int main(int argc, char **argv)
{
    ...
    while(1)
    {
		...
		for(i = 0; i < 100; i++){
			...
			len = recvfrom(server_socket, buffer, sizeof(struct timeval), 0, (struct sockaddr*)&server_addr, &length);

		    memcpy(&send_time[i], buffer, sizeof(struct timeval));
			//printf("\n>>>%d>>>>receive time is %ld>>>>>%ld>>>>\n", i, receive_time[i].tv_sec, receive_time[i].tv_usec);
			//printf("\n>>>%d>>>>>>>send time is %ld>>>>>%ld>>>>\n", i, send_time[i].tv_sec, send_time[i].tv_usec);
			free(buffer);
		}
